...
function extractNameFromTemplate(value) {
  ...
  return arr[1].concat(' ', arr[2].slice(0, -1));
}
...
function getRectangleString(width, height) {
  const lineGorizontal = '─'.repeat(width - 2);
  const spaces = ' '.repeat(width - 2);
  const top = `┌${lineGorizontal}┐\n`;
  const button = `└${lineGorizontal}┘\n`;
  const body = `│${spaces}│\n`;
  return top + body.repeat(height - 2) + button;
}
